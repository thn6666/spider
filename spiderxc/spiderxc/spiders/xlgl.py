...
# }
class XlglSpider(scrapy.Spider):
    name = "xlgl"
    allowed_domains = ["nmj.xlgl.gov.cn"]

    def start_requests(self):
        for page in range(1,235):
            url = f'http://nmj.xlgl.gov.cn/eportal/ui?pageId=605b0a4fc2ae49879c44844710f7999e&currentPage={page}&moduleId=fa41d45fdb6b40188b448c0209460238&staticRequest=yes'
            yield scrapy.Request(url=url)

    def parse(self, response):
        sel = Selector(response)
        list_items = response.xpath('//*[@id="fa41d45fdb6b40188b448c0209460238"]/div[2]/ul/li')

        for list_item in list_items:
            movie_item = MovieItem()
            url = list_item.xpath('./a/@href').get()
            movie_item['url'] = 'http://nmj.xlgl.gov.cn' + url
            movie_item['update_at'] = list_item.xpath('./span/text()').get()
            movie_item['title'] = list_item.xpath('./a/text()').get().strip()
    ...
    def parse_detail(self, response, **kwargs):
        ...
        list_items = response.xpath('//*[@id="xlCon"]')
        for list_item in list_items:
